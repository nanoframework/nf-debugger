...
namespace nanoFramework.Tools.Debugger
{
    ...
    public class BinaryFormatter
    {
        ...
        struct TypeDescriptorBasic
        {
            ...
            internal void SetType(Type t)
            {
                ...
                if (m_et == ElementType.PELEMENT_TYPE_VOID)
                {
                    ...
                    else if (t.GetTypeInfo().IsEnum) { m_et = ElementType.PELEMENT_TYPE_VALUETYPE; m_flags |= c_Enum; }
                    else if (t.GetTypeInfo().IsValueType) { m_et = ElementType.PELEMENT_TYPE_VALUETYPE; m_flags |= c_ValueType; }
                    else if (t.GetTypeInfo().IsInterface) { m_et = ElementType.PELEMENT_TYPE_CLASS; m_flags |= c_Interface; }
                    else if (t.GetTypeInfo().IsClass) { m_et = ElementType.PELEMENT_TYPE_CLASS; m_flags |= c_Class; }
                    else
                    {
                        goto Failure;
                    }
        ...
        class TypeHandler
        {
            ...
            internal void SetValue(object v)
            {
                if (v != null)
                {
                    ...
                    if (t.GetTypeInfo().IsEnum || t.GetTypeInfo().IsPrimitive || (t.GetTypeInfo().Attributes & TypeAttributes.Serializable) != 0)
                    {
                        m_value = v;
            ...
            internal int EmitSignature()
            {
                ...
                if ((mask & c_Signature_Type) == 0)
                {
                    ...
                    {
                        if (m_typeForced == typeof(Type) && m_value.GetType().GetTypeInfo().IsSubclassOf(m_typeForced))
                        {
                        }
                        else
                        {
            ...
            private void EmitSignature_Inner(int mask, TypeDescriptorBasic td, TypeDescriptor tdBase, object v)
            {
                ...
                if (td.Type.GetTypeInfo().IsSubclassOf(typeof(Type)))
                {
                    Type t = (Type)v;
            ...
            {
                if (m_type.Type.GetTypeInfo().IsSubclassOf(typeof(Type)))
                {
        ...
        class State
        {
            ...
            private State AdvanceToTheNextField()
            {
                while (m_fields_CurrentClass != null)
                {
                    ...
                    {
                        m_fields_CurrentClass = m_fields_CurrentClass.GetTypeInfo().BaseType;
                        m_fields_Fields = null;
                    }
                }
        ...
        //        {
        //            if (s_fOnlySerializableObjects && t.GetTypeInfo().IsSerializable == false) continue;
        ...
        static private void ConvertObject(object dst, object src)
        {
            ...
            for (int iField = 0; iField < fi.Length; iField++)
            {
                ...
                if ((fieldSrc.FieldType.GetTypeInfo().IsValueType || fieldSrc.FieldType.GetTypeInfo().IsEnum) && (!fieldSrc.FieldType.GetTypeInfo().IsPrimitive))
                {
        ...
        static public SerializationHintsAttribute GetHints(MemberInfo mi)
        {
            ...
            if (mi != null)
            {
                ...
                var lst = mi.GetCustomAttributes(false);
                bool fFoundSerializationHints = false;

                for (int iAttr = 0; iAttr < lst.Count(); iAttr++)
                {
                    object hintsSrc = lst.ElementAt(iAttr);
        ...
        static internal void AddType(Type t)
        {
            ...
            AddType(t.GetTypeInfo().BaseType);
