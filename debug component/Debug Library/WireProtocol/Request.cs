        ...
        internal async Task<IncomingMessage> PerformRequestAsync(CancellationToken cancellationToken)
        {
            ...
            while (retryCounter++ <= retries)
            {
                ...
                if (await outgoingMsg.SendAsync().ConfigureAwait(false))
                {
                    ...
                    if((this.outgoingMsg.Header.m_cmd == Commands.c_Monitor_Reboot) && retryCounter == 1)
                    {
                        // wait here for 
                    ...
                    CancellationToken cancelTaskToken = source.Token;
                    CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, cancelTaskToken);
